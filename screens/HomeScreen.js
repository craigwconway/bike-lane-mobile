...
import { Button, ButtonGroup, Input, Overlay, Text, ThemeProvider } from 'react-native-elements';
...
import { Report, ReportService, initialRegion } from '../services/ReportService';
...
export default class HomeScreen extends React.Component {
  ...
  getLocation = async () => {
    if (Platform.OS === 'android' && !Constants.isDevice) {
      ...
      if (status !== 'granted') {
        alert('To more effectivly use this app, go to Device Settings and enable Location services.');
        this.setState({ region: initialRegion() });
        this.setState({ loading: false });
      }else{
        let location = await Location.getCurrentPositionAsync({});
        let { latitude, longitude } = location['coords'];
        this.setState({ region: { latitude, longitude, latitudeDelta: MAP_SCALE, longitudeDelta: MAP_SCALE } });
  ...
  getMarkers = async (latitude, longitude, distance) => {
    let nearbyReports = await ReportService.findByProximity(latitude, longitude, distance);
    // let markers = nearbyReports.map(r => r.getMarker());
    this.setState({markers: nearbyReports});
  }
  ...
  handleCreateReport = async () => {
    ...
    const result = report.id ? ReportService.update(report) : ReportService.create(report);
    if(result.error){
      alert(result.error);
  ...
  handleMapPress = event => {
    if(event.nativeEvent.action !== 'marker-press' 
      ...
      this.setState({region: {...event.nativeEvent.coordinate, latitudeDelta, longitudeDelta}});
          ...
          <Overlay
            ...
            >
              <View>
                ...
                <Text style={{ marginLeft: 5, marginBottom: 14, fontSize: 16, fontWeight: 'bold', color: 'gray'}}>
                  How bad is it?
                </Text>
...
export function Loading(){
  return <View style={styles.container}><Text h1>Loading...</Text></View>
}
